...
TASKS_DB = os.getenv("TASKS_DB_WFS1", "./data/tasks_wfs1.json")
INCIDENTS_DB = os.getenv("INCIDENTS_DB_WFS1", "./data/incidents_table_wfs1.json")
ATTENDANCE_DB = os.getenv("ATTENDANCE_DB_WFS1", "./data/attendance_wfs1.json")
ROSTER_DB = os.getenv("ROSTER_DB_WFS1", "./data/roster_store_wfs1.json")
BRIEFING_DB = os.getenv("BRIEFING_DB_WFS1", "./data/briefings_wfs1.json")
SUMMARIES_DB = os.getenv("SUMMARIES_DB_WFS1", "./data/summaries_wfs1.json")
...
class BriefingSnapshot(BaseModel):
    station: Optional[str] = "WFS1"  # <--- IDENTIDAD WFS1
    date: str
    shift: str
    timer: str
...
def load_tasks_from_disk():
    ...
    print(f"🗂️ [WFS1] Cargadas {len(tasks_in_memory_store)} tareas")

def save_tasks_to_disk():
    store_write_json(TASKS_DB, list(tasks_in_memory_store.values()), "Update tasks WFS1")
...
def save_incidents_to_disk():
    store_write_json(INCIDENTS_DB, latest_incidents_table, "Update incidents WFS1")
...
def save_attendance_to_disk():
    store_write_json(ATTENDANCE_DB, attendance_store, "Update attendance WFS1")
...
def _append_briefing(data: dict):
    store_append_json(BRIEFING_DB, data, "Append briefing WFS1")
...
async def send_to_excel_online(data: BriefingSnapshot):
    ...
    print(f"📤 [WFS1] Enviando a Excel: {json.dumps(payload)}")
    try:
        async with httpx.AsyncClient() as client:
            await client.post(url, json=payload, timeout=15.0)
    except Exception as e:
        print(f"❌ Error Excel WFS1: {e}")
...
if __name__ == "__main__":
    ...
    uvicorn.run("main_wfs1:app", host="0.0.0.0", port=8003, reload=True)
